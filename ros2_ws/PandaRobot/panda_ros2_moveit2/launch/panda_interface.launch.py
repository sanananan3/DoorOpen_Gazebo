...
def generate_launch_description():
    ...
    # Initial joint positions file :
    initial_positions_config = load_yaml("panda_ros2_moveit2","config/initial_positions.yaml")
    initial_positions = {"initial_positions": initial_positions_config}
    ...
    door_hinge_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["door_hinge_controller", "-c", "/controller_manager"],
    )

    door_joint_state_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["door_joint_state_controller",  "--controller-manager", "/controller_manager"],
    )
        ...
        [
            ...
            RegisterEventHandler(
                OnProcessExit(
                    ...
                    on_exit = [
                        door_hinge_controller_spawner,
                    ]
                )
            ),
            RegisterEventHandler(
                OnProcessExit(
                    target_action = door_hinge_controller_spawner,
                    on_exit = [
                        door_joint_state_controller_spawner,
                    ]
            ...
            RegisterEventHandler(
                OnProcessExit(
                    target_action = door_joint_state_controller_spawner,
                    on_exit = [
